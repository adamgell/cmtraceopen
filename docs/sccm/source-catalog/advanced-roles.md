...
| `client-notification-bgb` | Server-side notification and management-point context; `BgbServer.log` observed in site server logs | Simple (SMS trace; descriptive, not rule-validatable yet) | `observed` | High: certificate, database, device, user, and notification payload | A failure outcome (`FailedClients` above zero) and a second push showing the PushID, TaskID and TaskGUID key is collision-safe; success is already observed with an exact server-to-client key match |
