...
| `client-notification-bgb` | Server-side notification and management-point context; `BgbServer.log` observed in site server logs | Simple (SMS trace; descriptive, not rule-validatable yet) | `observed` | High: certificate, database, device, user, and notification payload | An observed client notification request with its terminal outcome, and independently validated server-versus-client notification keys |
