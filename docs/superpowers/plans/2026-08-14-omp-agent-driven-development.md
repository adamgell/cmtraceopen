...
2. Run `python3 .omp/skills/cmtraceopen-dev/scripts/setup_skillset.py --check` before reading any curated external skill; require the exact approved skill-name set, current-user-owned and non-group/world-writable source entries, symlink-free source trees, and the repository-pinned complete-tree SHA-256 digests.
