...
`parse_args` accepts only `--home`, `--target`, and `--check`. There is no repository option because every approved source is home-relative.

`APPROVED_SKILLS` maps the 15 approved external skill names to paths relative to the operator's home directory:
...
`code-review.md` uses `model: "@reasoning"`, tools `[read, grep, glob]`, and autoloads `cmtraceopen-code-review`, `coderabbit-review-loop`, `contract-scoped-review`. Its closed schema requires `role: code-review`, `phase` in `[review_report, blocked]`, exact 40-hex `head_sha` and `base_sha`, `findings` (array), `gate_states` (object), `coverage` (array), and `blockers` (array). Every finding carries a portable repository-relative `file_line` in exact `path:positive-line` form plus nonempty mechanism, failure scenario, and severity. A clean `review_report` requires empty findings and blockers, nonempty coverage, and exactly the four passed mandatory gates. A completed blocked review always requires a blocker and empty gate states; source defects carry verified findings plus coverage, while evidence-only failures may leave findings and coverage empty.
...
Write focused tests that reject an empty productive payload for every role, accept representative productive and explicit blocked payloads, reject a role discriminator mismatch, and reject backslash, Windows-reserved, trailing-dot, and invalid-character paths. Code-review regressions must accept only the exact clean mandatory gate set for `review_report`; accept blocked source findings with nonempty coverage and evidence-only blocker output with empty findings and coverage; keep blocked `gate_states` empty; and reject `{"CI":"failed"}`, missing keys, extra keys, any non-`passed` clean value, clean findings, and blocked findings without coverage. Run the focused tests first and observe failure because `validate_agent_output.py` does not exist, then add only enough broker logic to make them pass.
...
- independent review raw `review_report` content-hashed and bound to exact head/base, with no findings or blockers, nonempty coverage, and exactly `gate_states: {"ci":"passed","coderabbit":"passed","charter_review":"passed","contract_conformance":"passed"}`; missing, extra, differently cased, or non-passed gates and zero transport exit alone do not pass. A blocked review instead keeps gate states empty and preserves either verified source findings with coverage or an evidence-only blocker; it cannot satisfy the independent-review gate;
- stable CodeRabbit helper raw JSON content-hashed and bound to the exact draft PR/current head/current `baseRefOid`, with `review_cleared: true` — a CodeRabbit review anchored to the exact head that either is APPROVED or, when advisory because `.coderabbit.yaml` sets `request_changes_workflow: false`, carries no unresolved CodeRabbit thread — and zero actionable unresolved bot threads; zero transport exit alone does not pass;
