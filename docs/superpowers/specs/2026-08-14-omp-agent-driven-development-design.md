...
The long-lived OMP process and reviewed checked-in broker/helper TCB may hold the provider credential, but candidate worktree code never inherits it. Main launches every policy-approved test, build, linter, formatter, or renderer command that can execute candidate worktree code as a bounded direct argument vector through `run_repo_check.py`. UI/Design browser checks are non-executable scenario strings, not repository commands: Main translates and executes accepted scenarios only through dedicated OMP browser tooling and records the actual visual/browser evidence. Scenario text is never passed to `Popen`, a shell, or `run_repo_check.py`, and browser observation cannot widen the repository-check policy with a dev-server or other executable. Every repository-check invocation supplies the manifest-bound canonical cwd device/inode, canonical Git common directory, branch, and expected HEAD. The broker independently lstats the literal cwd without following a symlink, observes the Git top level, common directory, worktree-list registration, attached branch, and HEAD before `Popen`, repeats the observation after process-group containment, and writes those observed values, not caller labels, into the artifact. A rename, symlink or directory replacement, primary-checkout substitution, registration/common-dir drift, branch drift, or HEAD drift is runner failure before evidence can pass. On POSIX, the broker constructs a minimal nonsecret environment rather than filtering a copied parent environment, starts a new process group, observes the group leader's exit without reaping it, kills every process that remains in that group while its identity is reserved, and then reaps the leader. Delayed in-group descendants therefore cannot mutate after the broker returns. The reviewed boundary assumes cooperating repository code: deliberate daemonization, `setsid`/`start_new_session`, process-group reassignment, or commands/dependencies that can detach block during Main's command/config review. This broker is not an OS sandbox, so hostile or unreviewed content blocks before execution. Unsupported non-POSIX execution blocks rather than weakening isolation. Main injects a credential only into the fixed model-inference or GitHub API process that requires it, never into candidate code.
...
- `.omp/skills/cmtraceopen-dev/scripts/lane_state.py`: a Python-standard-library helper that validates the manifest schema, persists canonical worktree path/device/inode/Git-common-dir/branch/HEAD bindings, revalidates them around worktree-consuming operations, performs atomic updates and head/base invalidation, and compares complete changed-path sets with a lane allowlist. It does not create worktrees or mutate GitHub.
- `.omp/skills/cmtraceopen-dev/scripts/check_command_policy.py`: the shared fail-closed executable and argument policy for proposed checks and broker execution;
- `.omp/skills/cmtraceopen-dev/scripts/validate_agent_output.py`: deterministic post-schema role, phase, productivity, path, and command validation;
- `.omp/skills/cmtraceopen-dev/scripts/run_repo_check.py`: the credential-scrubbed direct-command broker that binds execution and artifacts to the manifest-supplied worktree identity, Git common directory, branch, HEAD, and base.
...
The installer validates every source before target mutation and serializes cooperating writers with all distinct target-derived lock files inside a current-user-owned mode-`0700` directory. Before it reconciles links, it requires the exact repository-approved skill-name set and compares a deterministic SHA-256 digest of every path and regular-file byte in each complete source tree against the checked-in digest table. Every traversed source file and directory must be owned by the current user and not group/world writable, and nested symlinks or special files fail closed. The same complete-tree snapshot participates in the installer's before/after source-identity checks. For every literal target it computes both a stable normalized absolute lexical key and the current canonical alias key, deduplicates their lock-file hashes, acquires them in one globally sorted order, and releases them in reverse order. The lexical key makes repeated use of the same literal target contend even if an absent parent later appears as a symlink; the canonical key makes distinct lexical aliases of the same current target contend. It captures every approved entry with an `lstat`/`readlink`/`lstat` identity snapshot, derives drift only from that snapshot, records the nearest existing target ancestor, revalidates every recorded ancestor before creating each descendant, revalidates the target directory, complete entry set, and every preexisting entry before mutation, and verifies the complete curated link set and ancestor chain again before reporting success. Concurrent or ambiguous entries are preserved and fail closed.
...
- agent ID and role;
- canonical absolute worktree path, lstat directory identity (`device`, `inode`), and canonical Git common directory;
- attached branch name and independently observed local HEAD;
- allowed write paths;
...
Preflight derives and stores `PRIMARY_ROOT` as the canonical parent of the absolute Git common directory; a root supplied by the cold brief must resolve to the same path or dispatch blocks. Root-safety snapshots cover tracked, untracked, and ignored files under that derived primary checkout, primary-worktree Git controls, and the managed-worktree registration set. `filesystemSha256` excludes only `.git` and the orchestrator-managed top-level `.worktrees/`; user-owned ignored files elsewhere remain included. `gitControlsSha256` deliberately excludes unrelated active-branch refs and objects in shared Git storage. A coordinator-setup snapshot pair may be compared after coordinator registration but is never recorded as production evidence. For each production wave, Main creates and registers the complete issue-worktree set, allocates every lane, verifies each registration/clean allocation head, captures the before artifact, and records `rootSafety.stage2Waves[waveId]` with the sorted issue bindings, their immutable allocation bases/worktrees, and `managedWorktreesSha256` before any proposal or lane write. A lane may belong to only one Stage 2 wave. After every bound lane reaches `reviewing`, Main captures and records the after artifact while the same worktrees remain registered and before cleanup. The helper requires the same wave ID, sorted lane set, frozen bindings, registration digest, and exact before/after artifact hash. A mismatch leaves all wave lanes in `reviewing`, records the blocker, and prevents `ready_for_adam`.
...
2. Main allocates the issue branch, sole logical proposal owner, allowed paths, allocation base SHA, and lease. It strictly resolves the existing Git-registered worktree, independently observes its non-symlink directory device/inode, canonical Git common directory, attached branch, and HEAD, and persists all of them. Generic manifest validation thereafter compares persisted values without filesystem access, allowing a missing worktree lane to be blocked or abandoned; any changed-path, HEAD-update, evidence-recording, runner, or readiness operation revalidates the complete physical/Git binding before and after use.
...
4. Apply the role-specific first-proposal contract with the role profile's frontmatter output schema and `schemaMode: strict`; retry-exhausted invalid output blocks the lane. Because provider normalization strips regex and min/max/conditional constraints, Main also runs the parsed result through the checked-in `validate_agent_output.py` broker before consuming it. The broker requires the exact role/phase discriminator, rejects empty productive or mixed blocked/proposal payloads, applies the same portable path grammar as the lane manifest, and requires every proposed repository command to be a bounded direct `argv` array plus timeout rather than shell text. Separately, each UI/Design browser check must be a nonempty, control-free, non-executable scenario string of at most 4096 characters; productive UI output requires at least one scenario and blocked UI output requires none. Observed Git paths use a separate containment grammar so legal POSIX spaces and colons do not create false ownership failures; proposal and allowlist paths remain portable. The broker prints only `{"ok":true,"role":"ROLE"}` on acceptance; every rejection is nonzero and fail-closed.
5. Main independently inspects every proposal and broker-applied change. Before `replace` or `delete`, Main captures the byte-exact preimage, lstat identity, canonical parent identity, and Git index entry/stage state, then immediately re-reads and requires all four to match before mutation. For `create` and `replace`, Main canonicalizes every final changed target and requires an unambiguous existing path contained in the assigned worktree and matched by the persisted manifest allowlist. For `delete`, Main requires an obsolete tracked target matched by the persisted manifest allowlist, then requires that exact target to be absent and the same canonical parent identity to remain after application. A preimage mismatch, symlink, parent swap, or nonexistent-path ambiguity blocks before evidence is accepted. Main then runs `lane_state.py check-paths --manifest PATH --issue N`, which loads the allocation base and complete persisted allowlist from the validated lane record. The post-write check requires every changed path to exist except the exact approved delete path, whose absence is permitted only after the pre-application delete validation passed. This manifest-bound post-write check remains mandatory when an adversarial fixture path passed pre-application validation.
6. For Coder work, Main validates the proposed argument vector and timeout and runs the focused command through the credential-scrubbed repository-check broker with the manifest-bound `--expected-worktree-device`, `--expected-worktree-inode`, `--expected-git-common-dir`, `--expected-branch`, and `--expected-head-sha`. It accepts the content-hashed artifact only when the runner's independently observed cwd path/identity/common directory/branch/HEAD match the lane. The exact-head nonzero artifact is RED only when the proposed focused test/fixture ran and its expected assertion failed because the requested behavior was absent. A timeout, identity or registration mismatch, symlink/replacement, missing dependency or executable, import/setup or runner failure, unrelated test failure, zero exit, scenario-only observation, child claim, direct run, or shell invocation is not RED evidence.
7. The same logical owner returns the smallest contract-complete structured proposal and only proposed role-appropriate verification. Main applies the accepted proposal exactly; UI/Design returns non-executable browser-scenario strings and never claims observed browser evidence, while Tech Writer never claims its proposed checks ran.
8. Main independently inspects the diff, repeats the manifest-bound path check, runs focused GREEN and policy-approved documentation commands through the credential-scrubbed broker whenever worktree code can execute, translates and executes accepted UI scenarios only through dedicated OMP browser tooling, and records the actual visual/browser evidence. A scoped `git diff --check` proves whitespace only; unavailable required link or render validation blocks instead of passing on weaker evidence. Browser-scenario text is never passed to `Popen`, a shell, or the repository-check runner. The path check and evidence mutation both revalidate the stored worktree identity, Git registration/common directory, branch, and current HEAD before and after consumption; replacement content is never inspected or mutated. Children never mutate files, run commands or Git/GitHub operations, or read credentials.
9. Main may commit and push the verified coherent diff and open or update a draft PR without pausing.
10. Review proceeds contract first, adversarial second, mechanical third. Read-only reviewers inspect source and Main-supplied exact-head/current-base artifacts; they do not run commands. A completed review with source findings uses `phase: blocked`, binds both SHAs, carries verified findings and nonempty coverage plus at least one blocker, and keeps gate states empty. An evidence-only blocker such as a missing or stale mandatory gate may use the same phase with empty findings and coverage. Its closed `review_report` is clean only with no findings or blockers, nonempty coverage, and exactly `gate_states: {"ci":"passed","coderabbit":"passed","charter_review":"passed","contract_conformance":"passed"}`. The profile schema and deterministic post-parse broker enforce both branches and reject missing, extra, differently cased, non-passed, or stale gates.
11. Main runs the checked-in CodeRabbit helper twice to obtain a stable raw snapshot that includes `baseRefOid`, content-hashes that raw JSON, and continues review until the parsed snapshot binds the exact lane PR/head/current base, reports a draft PR, reports `review_cleared: true` — a CodeRabbit review anchored to the exact head that either is APPROVED or, when advisory because `.coderabbit.yaml` sets `request_changes_workflow: false`, carries no unresolved CodeRabbit thread — and contains zero unresolved non-outdated bot threads. A zero wrapper/gate exit is transport evidence only and never a clean verdict.
12. Independent charter review repeats at the exact head and current base until its content-hashed exact structured raw `review_report` binds both SHAs, contains no findings or blockers, has nonempty coverage, and reports the closed mandatory set `ci`, `coderabbit`, `charter_review`, and `contract_conformance`, each exactly `passed`. A blocked review preserves verified source findings with coverage or records an evidence-only blocker, always with empty gate states, and cannot become `independent_review: passed`. Raw-verdict validation rejects any missing, extra, differently cased, or non-passed gate before `independent_review` can enter `passed`. Changed bytes, stale head/base, or a new blocker sends the lane back to review.
...
- repository-controlled checks and local Git helpers cannot observe gateway, GitHub, provider, or cloud credentials inherited by Main;
