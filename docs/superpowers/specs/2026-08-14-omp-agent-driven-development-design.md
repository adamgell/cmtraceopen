...
Preflight derives and stores `PRIMARY_ROOT` as the canonical parent of the absolute Git common directory; a root supplied by the cold brief must resolve to the same path or dispatch blocks. Root-safety snapshots cover tracked, untracked, and ignored files under that derived primary checkout plus primary-worktree Git controls. `filesystemSha256` excludes only `.git` and the orchestrator-managed top-level `.worktrees/` directory; user-owned ignored files everywhere else remain included. `gitControlsSha256` covers primary-worktree Git controls while deliberately excluding unrelated active-branch refs and objects in shared Git storage. Before/after equality of both digests therefore detects primary-root mutation without false incidents from concurrent worktree branch activity.
...
4. Apply the role-specific first-artifact contract. Coder writes only the smallest focused failing test/fixture. Reducer Adversary is strictly read-only and returns an adversarial RED contract plus fixture/test proposal as text. Main independently inspects and approves that proposal, then dispatches Coder with sole lane ownership, the absolute worktree, and allowlist to materialize only the RED artifact. UI/Design first prepares the approved UI change plus proposed browser checks. Tech Writer first prepares the approved documentation change plus proposed source, link, and render checks.
