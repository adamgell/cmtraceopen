...
The installer validates every source before target mutation and serializes cooperating writers with all distinct target-derived lock files inside a current-user-owned mode-`0700` directory. For every literal target it computes both a stable normalized absolute lexical key and the current canonical alias key, deduplicates their lock-file hashes, acquires them in one globally sorted order, and releases them in reverse order. The lexical key makes repeated use of the same literal target contend even if an absent parent later appears as a symlink; the canonical key makes distinct lexical aliases of the same current target contend. It captures every approved entry with an `lstat`/`readlink`/`lstat` identity snapshot, derives drift only from that snapshot, records the nearest existing target ancestor, revalidates every recorded ancestor before creating each descendant, revalidates the target directory, complete entry set, and every preexisting entry before mutation, and verifies the complete curated link set and ancestor chain again before reporting success. Concurrent or ambiguous entries are preserved and fail closed.
