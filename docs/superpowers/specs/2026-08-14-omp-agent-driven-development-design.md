...
Preflight derives and stores `PRIMARY_ROOT` as the canonical parent of the absolute Git common directory; a root supplied by the cold brief must resolve to the same path or dispatch blocks. Root-safety snapshots cover tracked, untracked, and ignored files under that derived primary checkout, primary-worktree Git controls, and the managed-worktree registration set. `filesystemSha256` excludes only `.git` and the orchestrator-managed top-level `.worktrees/` directory; user-owned ignored files everywhere else remain included. `gitControlsSha256` covers primary-worktree Git controls while deliberately excluding unrelated active-branch refs and objects in shared Git storage. `managedWorktreesSha256` covers the orchestrator-managed worktree registrations and therefore legitimately changes when required issue worktrees are created or removed. A coordinator-setup snapshot pair may be captured and compared immediately around manifest setup after the coordinator registration exists, but it is never recorded as `rootSafety.stage2Before`. For Stage 2, Main first creates and registers the exact three issue worktrees, allocates their lanes, and verifies that every worktree is registered, clean, and at its allocation head. Main then captures and records the exact `stage2Before` JSON and SHA-256 before any child proposal or Main-applied lane work. After all child and Main work stops, Main captures `stage2After` while the same issue worktrees remain registered and before any cleanup. No issue worktree may be created or removed between the pair. Exact byte equality of the two JSON artifacts, including all three digests, is the root-safety gate; a mismatch blocks every lane.
