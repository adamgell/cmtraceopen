...
10. Review proceeds contract first, adversarial second, mechanical third. Read-only reviewers inspect source and Main-supplied exact-head/current-base artifacts; they do not run commands. Their closed `review_report` binds both SHAs and is clean only with no findings or blockers, nonempty coverage, and exactly `gate_states: {"ci":"passed","coderabbit":"passed","charter_review":"passed","contract_conformance":"passed"}`. The profile schema and deterministic post-parse broker reject missing, extra, differently cased, or non-`passed` gates; blocked review output instead keeps findings, gate states, and coverage empty. Reducer work routes semantic questions through Reducer Contract and false-story testing through Reducer Adversary.
