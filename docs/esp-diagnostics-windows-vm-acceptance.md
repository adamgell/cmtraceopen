...
| Click **Request missing permissions** | Exactly one zero-argument `graph_request_missing_permissions` call; other Graph actions remain locked while it runs; an HWND-parented WAM approval/denial or administrator-approval surface becomes visible instead of immediately returning the cached partial token; no ESP fetch is started. Record any credential step separately from the consent decision. |
...
| Caller attempts to supply `scopes` or another permission list | The production command has no caller-supplied permission argument and builds the complete five-Graph-scope union natively. The explicit upgrade uses the v2 consent properties and no `resource`; the development bridge rejects the protected command instead of forwarding input. |

Static review evidence must additionally distinguish the existing persisted-option startup connection, the Settings-only permission action, and ESP behavior. It must show the initial/default request retaining `resource=https://graph.microsoft.com`; the permission action requesting the fixed five short delegated Graph scopes plus standard OIDC identity/session scopes with `ForceAuthentication`, `wam_compat=2.0`, `prompt=consent`, an explicit interactive authority, and no `resource`; no caller-controlled scope input; direct ESP Graph operations using GET; bounded `$batch` operations using a POST envelope whose subrequests are GET-only; bounded retry/pagination/body limits; zero upgrade calls from startup/ESP refresh; and token/authorization-header redaction.
