...
fn payload_for(path: &Path) -> StoreArtifactPayload {
    ...
    if path.extension().and_then(|ext| ext.to_str()) == Some("json") {
        serde_json::from_str(&text).unwrap_or_else(|error| {
            panic!("{} is a valid store artifact payload: {error}", path.display())
        })
    } else {
    ...
    manifest["artifacts"]
        ...
        .map(|artifact| {
            ...
            StoreSourceArtifact {
                ...
                observed_at_utc: artifact["capturedUtc"].as_str().expect("capturedUtc").to_owned(),
...
fn load_scenario(scenario: &str) -> (StoreAnalysis, Value) {
    ...
    (analyze_store_bundle(&load_bundle(scenario, &manifest)), expected)
}
...
fn store_win32_handoff_with_matching_installer_outcome() {
    ...
    assert_eq!(transaction.installer_family, StoreInstallerFamily::StoreWin32);
