    ...
    manifest["artifacts"]
        ...
        .map(|artifact| {
            let content = artifact["relativePath"]
                .as_str()
                .map(|relative| {
                    std::fs::read_to_string(scenario_root.join(relative)).unwrap_or_else(|error| {
                        panic!("{relative} is readable: {error}")
                    })
                })
                .unwrap_or_default();
            Win32SourceInput {
...
fn assert_coverage(
    ...
    failures.require(actual_ids == want_ids, || {
        format!(
            "{scenario}: coverage ids are {actual_ids:?}, expected exactly {want_ids:?}"
        )
    });
    ...
    let counters: [(&str, u64); 2] = [
        ...
        ("fragmentRecords", u64::from(analysis.coverage.fragment_records)),
    ];
