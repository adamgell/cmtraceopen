...
fn push_unknown_schema(snapshot: &AutopilotSnapshot, findings: &mut Vec<IntuneFinding>) {
    ...
    let mut summary = if unsupported.is_empty() {
        format!(
            ...
            snapshot.capture.windows_build.as_deref().unwrap_or("unknown")
        )
    } else {
        format!(
    ...
    if let Some(detail) = unsupported.first().and_then(|document| document.detail.clone()) {
        summary.push(' ');
    ...
    push(
        ...
        finding(
            ...
            unsupported
                ...
                .chain(
                    build_unvalidated
                        .then(|| snapshot.coverage.iter().map(|entry| entry.artifact_id.clone()))
                        .into_iter()
                        .flatten(),
                )
                .collect(),
...
fn push_network_or_service_symptom(snapshot: &AutopilotSnapshot, findings: &mut Vec<IntuneFinding>) {
...
fn signal_evidence(snapshot: &AutopilotSnapshot, signal: AutopilotSignal) -> Vec<IntuneEvidenceRef> {
    snapshot
        .observations
        .iter()
