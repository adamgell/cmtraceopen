...
pub fn redacted_export_projection(snapshot: &AutopilotSnapshot) -> AutopilotSnapshot {
    ...
    for key in &mut projected.esp_linkage.matched_keys {
        if !is_token(&key.value) {
            key.value = stable_token("correlation", &key.value);
        }
    }
    ...
    for value in values {
            ...
            .any(|key| key.eq_ignore_ascii_case(&value.name))
        {
            if !is_token(&value.value) {
                value.value = stable_token("value", &value.value);
            }
        } else {
    ...
    match value.split_once('=') {
                ...
                .any(|key| key.eq_ignore_ascii_case(name)) =>
        {
            if is_token(raw) {
                ...
                format!("{name}={}", stable_token("value", raw))
            }
        }
        _ => {
            if is_token(value) {
...
mod tests {
    ...
    fn equal_values_mask_to_equal_tokens_so_correlation_survives() {
        let left = stable_token("value", "5CD1234ABC");
        let right = stable_token("value", "5CD1234ABC");
        assert_eq!(left, right);
        assert_ne!(left, stable_token("value", "5CD1234ABD"));
    }
    ...
    fn an_already_masked_whole_value_is_left_alone() {
        let token = stable_token("value", "abc");
