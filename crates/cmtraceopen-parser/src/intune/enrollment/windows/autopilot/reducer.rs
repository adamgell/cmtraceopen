...
pub fn reduce_autopilot_bundle(bundle: &AutopilotBundleInput) -> AutopilotSnapshot {
    ...
    let esp_linkage = reduce_esp_linkage(&observations, &ingest.esp_sessions, &handoff, &conflicts);
...
fn signal_observations(
    ...
    observations
        .iter()
        .filter(move |observation| observation.signal == signal)
}
...
fn detect_conflicts(
    ...
    if !ambiguous.is_empty() {
        ...
        conflicts.push(AutopilotConflict {
            ...
            detail: "A single correlation key resolves to more than one ESP session."
                .to_owned(),
...
fn reduce_esp_linkage(
        ...
        .any(|conflict| conflict.kind == AutopilotConflictKind::EspSessionIdentifier)
    {
        return AutopilotEspLinkage {
            ...
            esp_session_ids: sorted_unique(
                esp_sessions.iter().map(|session| session.session_id.clone()),
            ),
    ...
    if matched_keys.is_empty() {
        // ESP facts exist but nothing binds them. This is the time-only case
        // and is capped at low confidence by construction.
        return AutopilotEspLinkage {
            state: AutopilotEspLinkState::TimeOnlyCandidate,
