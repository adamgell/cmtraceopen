...
use super::models::{
    AutopilotCaptureMetadata, AutopilotEspSessionFact, AutopilotSectionKind, AutopilotSectionOutcome,
    AutopilotTimezoneState,
};
...
pub const AUTOPILOT_EVENT_PROVIDER: &str = "Microsoft-Windows-ModernDeployment-Diagnostics-Provider";
...
pub fn detect_document(content: &str) -> AutopilotDocumentDetection {
    ...
    let kind: AutopilotDocumentKind = match serde_json::from_value(
        serde_json::Value::String(declared_kind.clone()),
    ) {
        Ok(kind) => kind,
...
const VALIDATED_BUILD_PREFIXES: [&str; 6] = ["10.0.19", "10.0.22", "10.0.25", "10.0.26", "19", "22"];
...
pub fn classify_timezone(timezone: Option<&str>) -> AutopilotTimezoneState {
    ...
    if looks_like_offset || iana_zone_re().is_match(timezone) || windows_zone_re().is_match(timezone)
    {
...
mod tests {
    ...
    fn timezone_shapes_are_classified_without_a_timezone_database() {
        assert_eq!(classify_timezone(Some("UTC")), AutopilotTimezoneState::Declared);
        assert_eq!(
        ...
        assert_eq!(classify_timezone(Some("   ")), AutopilotTimezoneState::Missing);
    }
