...
fn collect_from_artifact(artifact: &StoreSourceArtifact, observations: &mut Vec<StoreObservation>) {
    ...
    match payload {
        StoreArtifactPayload::ImeText { text } => collect_from_ime_text(artifact, text, observations),
...
fn next_evidence_request(
    ...
    if state == StoreTransactionState::StoreWin32Handoff {
        return Some(
            "Collect the Win32 app installer log for the handed-off package".to_owned(),
        );
    }
    ...
    if !has_intune_intent {
        return Some(
            "Collect IntuneManagementExtension.log or the app assignment for this package".to_owned(),
        );
    }
...
mod tests {
    ...
    fn an_observation_without_any_identifier_stays_unkeyed() {
        let analysis = analyze_store_bundle(&[event_artifact(
            "appx",
            vec![appx_event("appx", 1, 404, &[("DeploymentOperation", "Register")])],
        )]);
