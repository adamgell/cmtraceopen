...
use crate::intune::evidence::{
    IntuneArtifactCoverage, IntuneArtifactStatus, IntuneErrorCode, IntuneEvidenceRef, IntuneFinding,
    IntuneFindingConfidence, IntuneNamedValue, IntuneObservationContext, IntuneSourceKind,
};
...
pub enum StoreArtifactPayload {
    ...
    PackageFacts { facts: Vec<StorePackageFact> },
    Assignments { assignments: Vec<StoreAssignment> },
    InstallerOutcomes {
...
mod tests {
    ...
    fn a_full_name_without_a_publisher_segment_yields_nothing() {
        assert_eq!(derive_family_from_full_name("Contoso.SynthApp_1.2.3.0"), None);
    ...
    fn two_concrete_installer_families_never_merge() {
        ...
        assert!(
            !StoreInstallerFamily::StoreWin32.is_compatible_with(StoreInstallerFamily::UwpProvisioned)
        );
        assert!(
