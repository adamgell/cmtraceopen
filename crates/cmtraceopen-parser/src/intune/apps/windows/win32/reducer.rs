...
mod tests {
    ...
    fn an_unmapped_return_code_is_a_failure_whose_meaning_is_not_asserted() {
        let analysis = analyze_win32_bundle(&[workload(&[
            ...
            record(&format!("[Win32App] Installation is done for app with id: {APP}, exit code: 0x80070643")),
        ])]);
    ...
    fn a_block_naming_two_apps_refuses_to_spread_a_key() {
        ...
        // nothing.
        let analysis = analyze_win32_bundle(&[workload(&[
            record(&format!("[Win32App] Processing app policy for app with id: {APP}")),
    ...
    fn observations_carry_the_shared_evidence_envelope() {
        ...
        assert_eq!(observation.context.access_state, IntuneAccessState::Available);
        assert_eq!(observation.context.sensitivity, IntuneSensitivity::Sensitive);
        assert_eq!(
            observation
                .context
                .source_timestamp
