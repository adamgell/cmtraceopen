...
use super::models::{
    ...
    ConfigurationResolution, ConfigurationServiceState, ConfigurationSetting, ConfigurationSnapshot,
    ConfigurationSourceStatement, INTUNE_CONFIGURATION_SCHEMA_VERSION,
};
use super::sources::{is_device_side, is_service_side, observation_from_event, observation_from_report};
...
fn reconcile_multiple_local(terminal: &[ConfigurationDisposition]) -> ConfigurationLocalState {
    ...
    if has(ConfigurationDisposition::Removed)
        && terminal
            .iter()
            .all(|d| matches!(d, ConfigurationDisposition::Removed | ConfigurationDisposition::Applied))
    {
...
fn source_statements(
    ...
    let mut grouped: BTreeMap<
        (Option<String>, ConfigurationEvidenceSide, ConfigurationDisposition),
...
fn ordering_is_contradictory(observations: &[ConfigurationObservation]) -> bool {
    ...
    for observation in observations.iter().filter(|obs| is_device_side(obs)) {
        let (Some(record_id), Some(instant)) =
            (observation.record_id, observation.occurred_at_utc.as_deref())
        else {
            continue;
        };
