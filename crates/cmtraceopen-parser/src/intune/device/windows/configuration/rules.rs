...
fn push_substantiated_conflict(snapshot: &ConfigurationSnapshot, findings: &mut Vec<IntuneFinding>) {
    let settings = select(snapshot, |setting| {
...
fn push_substantiated_supersedence(
    ...
    push_finding(
        ...
        finding(
            ...
            vec![
                "Confirm the replacing configuration source carries the value you intended.".to_owned(),
            ],
            evidence_where(&settings, |_| true),
            Vec::new(),
        ),
