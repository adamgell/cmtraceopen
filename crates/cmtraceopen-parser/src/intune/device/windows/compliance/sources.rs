...
use crate::intune::evidence::{
    IntuneArtifactCoverage, IntuneArtifactStatus, IntuneErrorCode, IntuneNamedValue,
    IntuneObservationContext, IntuneTimestamp,
};
...
fn absorb_envelope(text: &str, input: &mut ComplianceInput) -> Option<DecodeProblem> {
    ...
    for record in envelope.records {
        if let Err(error) = absorb_record(kind, record, input) {
            return Some(DecodeProblem {
                status: IntuneArtifactStatus::ParseFailed,
...
fn absorb_record(
    kind: EnvelopeKind,
    record: Value,
    input: &mut ComplianceInput,
) -> Result<(), serde_json::Error> {
    match kind {
        EnvelopeKind::WindowsEvents => input.events.push(serde_json::from_value(record)?),
        EnvelopeKind::SettingReports => input.setting_reports.push(serde_json::from_value(record)?),
        EnvelopeKind::CustomCompliance => {
            input.custom_compliance.push(serde_json::from_value(record)?)
        }
        EnvelopeKind::ServiceResults => input.service_results.push(serde_json::from_value(record)?),
        EnvelopeKind::AccessDecisions => {
            input.access_decisions.push(serde_json::from_value(record)?)
        }
        // Last one wins; a bundle is expected to declare device context once.
        EnvelopeKind::DeviceContext => input.device_context = Some(serde_json::from_value(record)?),
    }
    Ok(())
}
...
pub(super) fn parse_error_code(raw: &str) -> Option<IntuneErrorCode> {
    ...
    if let Some(hex_match) = hex_code_re().find(raw) {
        ...
        let decimal = u64::from_str_radix(hex.trim_start_matches("0x").trim_start_matches("0X"), 16)
            .ok()
            .map(|value| value as i64);
        return Some(IntuneErrorCode {
            raw: raw.to_owned(),
            decimal,
            hex: Some(format!("0x{:08X}", decimal.unwrap_or_default() as u32)),
        });
    }
...
pub(super) fn classify_setting_report(report: &NormalizedSettingReport) -> Option<SettingObservation> {
...
/// Wrap an already-normalized UTC string as a timestamp, keeping the raw text.
fn synthetic_timestamp(raw: &str) -> IntuneTimestamp {
    ...
    IntuneTimestamp {
        ...
        kind: if normalized.is_some() {
            crate::intune::evidence::IntuneTimestampKind::Utc
        } else {
            crate::intune::evidence::IntuneTimestampKind::Invalid
        },
