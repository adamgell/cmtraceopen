...
use super::sources::{
    classify_event, classify_setting_report, decode_bundle, ComplianceSignal, ComplianceSourceInput,
    SettingObservation,
};
use crate::intune::evidence::{IntuneEvidenceRef, IntuneTimestamp};
...
pub fn analyze_compliance(input: &ComplianceInput) -> ComplianceSnapshot {
    ...
    let access_impact = reduce_access(input, &reporting, &device_context);
...
fn reduce_local(input: &ComplianceInput, generated_at: Option<DateTime<Utc>>) -> ComplianceLocalPhase {
...
fn reduce_reporting(
    ...
    for event in &input.events {
        ...
        evidence.push(event.context.evidence_ref.clone());
        // A failure is the outcome worth keeping: a later "queued" does not undo
        // an observed submission failure in the same bundle.
        if phase.state != ComplianceReportState::Failed {
            phase.state = state;
        }
...
fn reduce_access(
    ...
    for fact in &input.access_decisions {
        ...
        let matched = reporting
            .service_results
            .iter()
            .filter(|result| keys_match(fact, result, device_context))
            .filter(|result| {
                match (
        ...
        let linkage = if !matched.is_empty() {
            ...
            .service_results
            .iter()
            .any(|result| keys_match(fact, result, device_context))
        {
...
///
/// The device-context fallback applies **only** to a record that declares no
/// identity of its own. Without that guard the fallback is loop-invariant: it
/// compares the decision to the snapshot's single device key, which is true for
/// essentially every real bundle, and would therefore match every service record
/// including one that explicitly names a different device or user.
fn keys_match(
    fact: &ComplianceAccessFact,
    result: &ComplianceServiceResult,
    device_context: &ComplianceDeviceContextView,
) -> bool {
    ...
    if same(&fact.device_key, &result.device_key) || same(&fact.user_key, &result.user_key) {
        return true;
    }
    result.device_key.is_none()
        && result.user_key.is_none()
        && same(&fact.device_key, &device_context.device_key)
}
...
mod tests {
    ...
    fn a_stale_service_record_never_changes_the_local_state() {
        ...
        let snapshot = analyze_compliance(&input);
        assert_eq!(snapshot.aggregate.state, ComplianceAggregateState::Compliant);
        assert_eq!(
