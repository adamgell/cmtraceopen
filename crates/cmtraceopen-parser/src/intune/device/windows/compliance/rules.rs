...
/// says so explicitly.
fn push_user_scoped_not_evaluated(snapshot: &ComplianceSnapshot, findings: &mut Vec<IntuneFinding>) {
...
fn push_contradictory_local_evidence(
    ...
    push(
        ...
        finding(
            ...
            &format!(
                "{} setting(s) carry disagreeing verdicts, {} claim an evaluation time after this evidence was collected, and {} were described with conflicting identifiers. No verdict was chosen for the settings whose sources disagreed.",
                contradictory.len(),
                out_of_order.len(),
                mismatched_ids.len()
            ),
...
fn push_access_denied_correlated(snapshot: &ComplianceSnapshot, findings: &mut Vec<IntuneFinding>) {
    ...
    let mut evidence = collect_evidence(
        denials
            .iter()
            .flat_map(|decision| decision.evidence.iter().chain(decision.matched_evidence.iter())),
    );
    normalize_evidence(&mut evidence);
