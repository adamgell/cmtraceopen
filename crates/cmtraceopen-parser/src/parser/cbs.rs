...
fn build_entry_from_caps(caps: &regex::Captures<'_>, file_path: &str) -> Option<LogEntry> {
    ...
    let timestamp = chrono::NaiveDate::from_ymd_opt(year, month, day)
        ...
        .map(super::local_wall_clock_millis);
...
mod tests {
    ...
    fn test_timestamp_is_the_local_wall_clock_and_not_a_utc_promotion() {
        ...
        let lines = ["2024-01-15 08:00:00, Info                  CBS    Exec: Processing package"];

        let (entries, _) = parse_lines(&lines, "C:/Windows/Logs/CBS/CBS.log");
