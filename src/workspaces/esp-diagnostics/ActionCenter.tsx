...
function Finding({ finding, graphNames }: FindingProps) {
  // Findings are generated by the backend reducer, which has no Graph overlay, so
  // they cite workloads by their raw GUID identifier. Resolve those to friendly Graph
  // names here (the overlay is frontend-only) so the Action Center reads app names.
  const title = resolveEspIdentifiers(finding.title, graphNames);
  const summary = resolveEspIdentifiers(finding.summary, graphNames);
  return (
    <article
      style={{
        display: "grid",
          ...
          <div style={{ marginTop: 6 }}>
            ...
            >
              {finding.recommendedChecks.map((check, index) => (
                <li key={`${finding.findingId}:check:${index}`}>
                  {resolveEspIdentifiers(check, graphNames)}
                </li>
              ))}
            </ol>
          </div>
