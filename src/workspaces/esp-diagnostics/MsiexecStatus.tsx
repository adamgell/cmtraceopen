...
function uniqueEvidence(
  ...
  return all.filter(
    (evidence, index) =>
      all.findIndex((candidate) => candidate.evidenceId === evidence.evidenceId) ===
      index,
  );
}
  ...
  return (
    ...
    >
      <div style={{ padding: "9px 11px", borderRight: `1px solid ${tokens.colorNeutralStroke2}` }}>
        ...
        <div style={{ marginTop: 5, fontFamily: LOG_UI_FONT_FAMILY, fontSize: 12, fontWeight: 650 }}>
          {process.executableName} · PID {process.pid}
        </div>
        <div style={{ marginTop: 3, color: tokens.colorNeutralForeground2, fontFamily: LOG_MONOSPACE_FONT_FAMILY, fontSize: 10 }}>
          {process.parentPid === null ? "Parent unknown" : `Parent PID ${process.parentPid}`}
        </div>
        <div style={{ marginTop: 2, color: tokens.colorNeutralForeground3, fontFamily: LOG_MONOSPACE_FONT_FAMILY, fontSize: 10 }}>
      ...
      <div style={{ minWidth: 0, padding: "9px 11px", borderRight: `1px solid ${tokens.colorNeutralStroke2}` }}>
        ...
        {rawIdentifier ? (
          <div style={{ marginTop: 2, overflowWrap: "anywhere", color: tokens.colorNeutralForeground3, fontFamily: LOG_MONOSPACE_FONT_FAMILY, fontSize: 10 }}>
        ...
        <div style={{ marginTop: 5, color: tokens.colorNeutralForeground2, fontSize: 10, lineHeight: "14px" }}>
          {correlation.reason}
        </div>
        <code
...
export function MsiexecStatus({ snapshot }: MsiexecStatusProps) {
  const observed = snapshot.installerCorrelations.flatMap((correlation) =>
    correlation.processObservations.map((process) => ({ correlation, process })),
  );
    ...
    >
      ...
      >
        ...
        <strong style={{ fontFamily: LOG_MONOSPACE_FONT_FAMILY, fontSize: 10 }}>
          {observed.length} active {observed.length === 1 ? "process" : "processes"}
        </strong>
      </div>

      {observed.length === 0 ? (
        <div style={{ display: "flex", alignItems: "center", gap: 8, minHeight: 52, padding: "0 12px", color: tokens.colorNeutralForeground2, fontFamily: LOG_UI_FONT_FAMILY, fontSize: 12 }}>
          <PulseRegular aria-hidden="true" />
          <strong>No active MSI installer process observed</strong>
          <span style={{ color: tokens.colorNeutralForeground3 }}>
            The status reflects the latest process sample; it is not proof that installation has completed.
          </span>
        </div>
      ) : (
