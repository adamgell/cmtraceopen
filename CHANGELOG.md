...
- **README and CONTRIBUTING linked documents that no longer exist**: `219536ff` removed `DSREGCMD_TROUBLESHOOTING.md` (moved to the wiki) and `FEATURE_IMPROVEMENTS.md` (stale roadmap) but left the links behind, so the README sent every visitor to a guide that is not in the tree. The README now points at the wiki page that holds the guide, and CONTRIBUTING lists the wiki instead of two dead entries.
