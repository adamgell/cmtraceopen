...
- **The redaction contract binds at the crate boundary (#550)**: ADR-004 revision 1 settles it — the audit found the previously accepted boundary bound at almost no real export surface, so a masked screen sat one button away from writing cleartext to a file of the operator's choosing. Every published analysis type is now constructible only in projected form, with the preserving variant behind an explicitly named local-only entry point, and the enforcement is structural through nested types rather than a call each egress must remember. `docs/architecture/decisions/ADR-004-redaction-scope-revision-1.md` carries the eight rulings and their open questions.
