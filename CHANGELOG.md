...
- **ADR-005 is confirmed as accepted (#539, #635)**: The decision that the frontend owns explicit event loads - with the backend-owned paging path deferred behind stated criteria - was recorded as *proposed, confirm at merge*, and the changes it required are in `main`: the per-request cancel flag (`evtx_cancel_channel_query`), the channel read loop stopping between batches, a stopped scan surfacing as `EvtxCoverageGapKind::Cancelled` that says the channel may hold more, and Stop offered in place of Load while a load runs. The record now says so and cites them. The epic ledger item that tracked this reads as deferred with criteria, as its consequences section states.
