...
- **Opening several files at once shows their modified times (#662)**: The multi-file lane built its sidebar entries with a modified time of `0`, which the sidebar renders as "Modified time unavailable", so every file in a multi-file or aggregate open read as unknown while a single-file open showed real times for the same files. A parse now reports the modified time it already stat'ed for the size, and the lane carries it; a platform that cannot supply one still reads as unknown rather than as epoch zero ([#662](https://github.com/adamgell/cmtraceopen/issues/662)).
