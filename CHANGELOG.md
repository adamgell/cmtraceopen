...
- **A failing workspace no longer takes the whole window**: The active workspace renders inside an error boundary, so a workspace that throws shows a panel naming it and its error while every other workspace stays reachable. The splash screen is also cleared on a window-level error, since a throw during the first render previously left it up indefinitely — indistinguishable from a slow start ([#698](https://github.com/adamgell/cmtraceopen/issues/698)).
