...
- **Typed sensitive values below the narrative literal floor (#646)**: A value the leaf classified as sensitive, but which fell below the six-byte floor that keeps short narrative literals out of the scrubber, could survive into an export. Typed values now take their own insertion path rather than being filtered by a rule written for prose.
