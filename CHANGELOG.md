...
- **Saved sessions are read through the backend, and the webview's file grants are narrowed (#681)**: Restoring a session read its path through the `fs` plugin, and that path comes from the recent-sessions list rather than from a dialog — so no picker had extended the plugin's scope for it, and the read was refused after a restart while it worked in the run that saved it. The read is now a `read_session_file` command, which accepts only the extension the save dialog writes and a bounded size; the shape validation stays where it was, in the frontend. The frontend's `fs:read-all`/`fs:write-all` grants — 25 commands where the app calls two — are replaced by `fs:allow-read-text-file` and `fs:allow-write-text-file`. Those grants carry commands, not paths, so the plugin's scope is untouched by the swap and the file and session pickers keep working through the dialog's own runtime grant.
