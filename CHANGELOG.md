...
- **Deployment folder scan is bounded**: The recursive scan behind "analyze deployment folder" now stops at a depth bound and refuses to enter symlinks, and caps the number of collected paths. It previously followed directory links and had no limit at all, in a synchronous command — a link to an ancestor recursed until the stack ran out, and a deep tree froze the window for the duration ([#702](https://github.com/adamgell/cmtraceopen/issues/702)).
