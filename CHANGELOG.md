...
- **PowerShell quoting in the elevation path (#682)**: Command strings in the Secure Boot script runner are built with `ps_single_quote`, so a temp path containing an apostrophe (a user named `O'Brien`) no longer terminates the PowerShell string early and turns path text into syntax inside the process that runs elevated.
