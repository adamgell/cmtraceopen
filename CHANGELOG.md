...
- **Multi-file open: modified time is per file (#662)**: A multi-file source showed `Modified time unavailable` for every entry, because the aggregate path discarded each file's own modified time and nothing downstream could recover it. Each entry now carries the modification time the file system reported for it, when the file was read: a cached multi-file load reads no file and reports none, so the sidebar keeps saying the time is unavailable rather than inventing one.
