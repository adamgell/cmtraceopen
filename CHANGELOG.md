...
- **Secure Boot elevation passes its paths as arguments (#682)**: The elevated launcher built PowerShell command text by interpolating four temp paths into single-quoted strings, and `%TEMP%` contains the user name - so a user named `O'Brien` produced a path whose apostrophe ended the string early, at minimum failing the elevation and at worst letting attacker-influenced text become PowerShell syntax inside an elevated process. The wrapper and the launcher are now static scripts that take their paths as `param()` arguments, and the launcher's argument vector is built as data, so no path is command text at any point and the class is removed rather than one instance escaped.
