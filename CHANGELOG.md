...
- **Deployment folder scan is bounded, and says when it was**: The recursive scan behind "analyze deployment folder" now stops at a depth bound, caps the collected paths, and does not descend a directory symlink, in a synchronous command that previously followed links and had no limit at all. It also reports which bound it hit, so a partial scan is not presented as the folder's contents: the workspace states "Scan incomplete" with the bounds named, and the status bar marks the count ([#702](https://github.com/adamgell/cmtraceopen/issues/702)).
