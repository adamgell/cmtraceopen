...
- **MAC profile payloads are no longer truncated by a brace in a value**: The parser that reads `mcx_preference_settings` counted `{` and `}` without regard for string literals, so a lone brace inside a quoted value — a template, filter expression or regex, all ordinary in an MDM payload — ended the block early and silently dropped every entry after it. The scan is now string-aware and respects escaped quotes ([#706](https://github.com/adamgell/cmtraceopen/issues/706)).
