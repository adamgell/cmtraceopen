...
- **Deployment folder scan is bounded and refuses to follow links (#702)**: `collect_log_files` recursed without a depth or result bound and followed directory symlinks, from a synchronous command that holds the main thread: a link back to an ancestor recursed until the stack ended the process, and a deep tree froze the window for as long as it took. The walk now reads each entry without following it and refuses reparse points, stops at a depth budget of 32 and a file budget of 5,000, and records which budget it stopped at on the analysis result. The workspace shows that, because logs past the bound are not part of the analysis — the counts below are of what was read, not of the folder.
