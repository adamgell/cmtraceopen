...
- **Linux Rust CI job**: `Check & Test (Rust)` failed while linking, with `ld` terminated by signal 7 (SIGBUS), on three runs in one day — two merges to `main` and a pull request — killing a different test binary each time and never reaching the parser-crate or audit steps. SIGBUS from `ld` is the shape of a memory-mapped write failing when the backing store cannot take it, and the job links several large test binaries after a cold build; the runner's disk was not measured at failure time, so this is the shape of the failure rather than an established cause. The job now reclaims the runner's preinstalled SDKs and toolchains first, none of which it uses, and prints `df -h /` so a recurrence carries the disk state.
