...
- **A file dated 1970 no longer reads as "Modified time unavailable"**: The sidebar formatter guarded with `!unixMs`, which rejects a legitimate zero timestamp. The backend maps a file's mtime through `duration_since(UNIX_EPOCH)`, so a zeroed mtime arrives as `Some(0)` rather than absent. The guard now tests for `null`, and the sidebar's four pure helpers have tests.
