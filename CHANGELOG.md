...
- **Exported field names are masked when they are not field names (#661)**: The redaction projection
  masked a named value's *value* but copied its *name* verbatim, so a capture carrying a user principal
  name in the name slot published that identity from a projection whose purpose is not to carry one. A
  name is exported only when it is one this build reads, a field the corpus carries, or a positional
  form (`Message5`, `HexInt1`) — and the positional suffix is bounded at three digits, because an
  unbounded one let `Message12345678901234567890` carry a serial past the same check.
