...
- **DsRegCmd capture bundle hand-off (#628)**: The live capture bundle is the analyzer's input, so it stays raw on disk — masking it where it is stored would change what the rules conclude rather than what anyone publishes. What was missing was the hand-off: the sidebar showed the bundle root and invited a support engineer to archive it, with the tenant id, the domains, the device id, the thumbprint, the user principal name and the user SID sitting in clear inside it, including occurrences no shaped rule reaches such as the tenant id in a registry key path. The sidebar now offers **Export shareable bundle**, which writes a projected copy of the same files under the same relative paths and shows it. The projection is the analysis path's own classification rather than a second list of identity fields, and the projected manifest records that it is a projection. A bundle with no command output is refused rather than written, because an artefact that looks projected without being projected is the one failure this hand-off must not produce. A bundle holding an entry that is neither a file nor a folder is refused too: a link cannot be followed without reading a target that may sit outside the bundle, and cannot be skipped without dropping an artifact the projection promises to carry.
