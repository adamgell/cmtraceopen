...
- **Frontend filesystem grants narrowed to the commands it uses (#681)**: `fs:read-all` and `fs:write-all` each grant the plugin's whole command surface - 14 read and 11 write - where the frontend calls two: `readTextFile` for the ESP log and session-file opens, and `writeTextFile` for the session save. Both are now the per-command permissions. The plugin's runtime scope is untouched, so the file pickers cannot break: `tauri-plugin-dialog` already `allow_file`s the path its own dialog returned, which is why this is command-count hygiene rather than a behaviour change. Removing the broad grants without replacing them would have broken the one read that takes its path from the recent-sessions list rather than from a dialog.
