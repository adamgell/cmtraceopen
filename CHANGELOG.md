...
- **Insufficient evidence no longer reads as "No issues detected"**: The event diagnosis panel mapped five outcomes onto four labels, collapsing `insufficientEvidence` — a coverage gap — onto the same wording as `noFindings`, a clean result. The two now read distinctly, and the test that asserted the shared wording has been replaced with a distinctness invariant ([#714](https://github.com/adamgell/cmtraceopen/issues/714)).
