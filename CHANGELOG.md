...
- **The file-manager launches record why they are unbounded (#684)**: `reveal.rs` and the macOS System Settings deep link spawn `explorer`, `open` and `xdg-open` without waiting for the launcher to exit, which is correct - `spawn` returns immediately and the handle is discarded, so there is no wait to bound and a deadline would not close the window the user asked for. That was true before and unstated; the reason now sits at each launch, which is what the issue asks for so the remaining unbounded sites are a decision rather than a drift.
