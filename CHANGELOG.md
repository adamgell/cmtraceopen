...
- **Removed two capability flags nothing read, and used the live-acquisition helper that already existed**: `WorkspaceCapabilities.liveAcquisition` and `.multiFileDrop` were declared, typed, documented, and never read by application code. The ESP workspace also inlined `currentPlatform === "windows"` three times beside an exported, tested `supportsEspLiveAcquisition` helper it never called.
