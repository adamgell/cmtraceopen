...
- **The decisions index lists ADR-005 (#539)**: The table of architecture decisions in `docs/architecture/decisions/README.md` stopped at ADR-004, so the decision governing a very large explicit event load was not reachable from the index that exists to list it. Its status is quoted from the ADR file itself rather than asserted here, because the confirmation that ADR asks for is carried by #679 and has not landed yet.
