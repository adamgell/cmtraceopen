...
- **Evidence collector tests now run in CI**: The 434-line Pester suite for the evidence collector had never been executed by any workflow, and one of its assertions could not have passed anywhere — it pinned a literal wall-clock time against a helper that embeds the current UTC time. The assertion now checks the id's shape and that its digits are invariant ASCII under a non-Gregorian culture, which is what the case is named for. 29 tests, all passing, wired into the Windows job.
