...
- **macOS profile payloads no longer stop at a brace inside a quoted value (#706)**: The block scanner counted `{` and `}` character by character with no notion of string literals, so a single closing brace inside a value ended the block there and every setting after it was dropped without a word — a payload with six settings rendered one of them, its value cut mid-string, and nothing reported the loss. Both counters — the scan behind `parsePayloadData` and the line collector used for nested dicts and arrays — now track quoted regions with backslash escapes handled, so a filter expression, a regex, or a `{0}` template placeholder is data rather than structure.
