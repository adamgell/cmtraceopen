...
- **The webview carries a content security policy (#681)**: `tauri.conf.json` had `"csp": null`, so Tauri injected nothing and the render path - the one place crafted log text reaches the DOM - had no policy between it and the webview's other authority. The app now ships `default-src 'self'` with the three allowances its own code needs, each checked against the built artifact rather than assumed: `script-src 'self'` because the production build emits **no** inline scripts (the one inline `<style>` block is the splash, and 2039 React inline styles are why `style-src 'unsafe-inline'` is required), and `connect-src 'self' ipc: http://ipc.localhost` because that is the origin `tauri` itself builds IPC requests against, in the `tauri` crate rather than this repository (`tauri/src/ipc/protocol.rs`). A `devCsp` is set too: the schema says the production value is injected on dev when it is absent, and Vite's HMR needs inline scripts and a websocket the production policy correctly refuses.
