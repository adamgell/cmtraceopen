...
- **The file-manager launches record why they are unbounded (#684)**: `reveal.rs` and the macOS System Settings deep link spawn `explorer`, `open` and `xdg-open` without a deadline, which is correct - they hand off to an interactive shell and outlive the call, so a deadline would close the window the user asked for. That was true before and unstated; the reason now sits at each launch, which is what the issue asks for so the remaining unbounded sites are a decision rather than a drift.
