...
- **Autopilot export no longer leaves a short typed identity in narrative (#736)**: A value read from an identity field - `deviceName`, `serialNumber`, `tenantDomain` - was masked where it was typed and left in the clear when the same value appeared in narrative, whenever it was shorter than the six-byte floor that keeps prose out of the scrubber. That floor describes a value that labels something, so identity fields now hold to a four-byte floor while display names such as the admin-authored profile name keep the six - the line #646 drew for the DsRegCmd leaf. The trade is deliberate and it is paid in narrative: a four-byte device name can also be an ordinary word, and scrubbing it costs readable evidence where that word appears.
