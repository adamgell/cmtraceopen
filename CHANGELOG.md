...
- **The DsRegCmd analyzer no longer reads the wall clock (item 3 of #738)**: `derive_capture_confidence` called `Utc::now()` to judge how old a capture was, so the same `dsregcmd /status` text produced different output depending on when it was parsed — against the project's rule that the parser crate is pure and extraction is reproducible. The parser crate performs no I/O and reads no clock; the evaluation instant is now a parameter, supplied at the application boundary, exactly as the bundle evidence already was. The crate's own tests also become deterministic inputs instead of depending on when they ran. **`cmtraceopen-parser` goes to 0.3.0**: the evaluation instant is a new required argument, so the published 0.2.0 signatures are broken for downstream callers. The crate's README states that the public API may change in a minor release while it remains at `0.x`, so this ships as a minor bump rather than a compatibility shim — and `src-tauri`'s `version = "0.2"` requirement moves with it.
