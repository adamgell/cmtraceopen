...
- **Timeline lane hover readout never appeared**: The brush overlay covers the swim lanes so it can capture a drag, and it also swallowed every pointer event the lane canvas needed: `elementFromPoint` at a lane centre resolved to the overlay, so hovering a bucket never produced the `"<n> rows · <m> errors"` corner readout and the canvas double-click hook could not fire. The canvas now exposes a `bucketAtPoint` hit test through a ref, the overlay forwards pointer position and leave, and the workspace owns the readout; the unreachable canvas hover props are gone.
- **Quick Stats severity cards could not be turned off**: Clicking a card filtered the list but never left the card active — the store's `hasActiveFilter()` counts filter clauses only, while the card applies `filteredIds` directly, so the external-clear effect reset the card on the same commit (the outline stayed 1px and a second click re-applied the filter instead of clearing it). The card now records the ids it applied, and the effect resets it only when the store's filter result is cleared or replaced by something else.
- **Sidebar "Merge into Timeline" kept the previous filter**: The merge replaces the whole source, but it was the only sidebar action that did not clear the active filter, so clause filters from the previous view stayed applied to the merged stream.
- **DsRegCmd timeline rows without a timestamp**: `Previous PRT attempt`, `Azure AD PRT update` and `Client reference time` fell back to the `Not Reported` placeholder instead of being omitted, so a capture with no timestamps still rendered three placeholder rows and the documented "No timeline-friendly timestamps were found in this capture." empty state was unreachable. A timeline row now carries a real timestamp or is dropped.
- **DsRegCmd capture-confidence qualifier mangled names and skipped two boxes**: `qualifyByCaptureConfidence` lower-cased the first character of the detail it prefixed, rendering `mDM visibility…`, `pRT present…` and `nGC is Yes…`, while `Current phase` and `Join posture` never carried the qualifier at all. The qualifier no longer re-cases the copy and both conclusion boxes route through it (`Capture trust` states the confidence itself and stays unqualified).
- **Debug IPC bridge dropped a request split across TCP segments**: The development bridge answered the first socket read, so a well-formed call whose body arrived in a second segment produced `request parse error: EOF while parsing a value` (or reset the connection) — roughly one call in four against a live `npm run app:dev` session. It now reads until the declared `Content-Length` is satisfied, bounded by a maximum request size.
- **Collector Pester suite was timezone-dependent**: `scripts/collection/tests/Invoke-CmtraceEvidenceCollection.Tests.ps1` mocked a local-kind clock while `New-CollectorBundleId` stamps UTC, so the bundle-id expectation encoded the runner's offset and failed on any host that is not UTC (one of 29 tests here, at UTC-0400). The mocked clock is now an explicit UTC instant.
