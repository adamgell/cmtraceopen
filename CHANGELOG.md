...
- **IME timestamps take the offset in force when the log was written (#738)**: The Intune Management Extension log carries no timezone in its `time=` field, and the fallback offset came from the machine's clock at read time. A log from the other side of a DST transition was an hour out, in the display column and in the stored epoch together. The offset is now resolved for the record's own instant, through the same zone handling the servicing parsers use; a clock the zone cannot place keeps the previous fallback rather than dropping the column, and the crate's `parser/mod.rs` supplies both.
