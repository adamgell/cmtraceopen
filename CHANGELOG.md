...
- **A live capture no longer reports success while its evidence writes fail (#760)**: Each evidence phase was written behind a series of silently-ignored failures - a directory that could not be created, a value that could not serialize, a write that was discarded - and the capture returned `Ok` regardless, so a bundle could ship listing evidence it did not contain. The three phases now report those failures the way the manifest beside them already did.
