...
use super::intake::{
    catalog_entry_id, is_reparse_point, logical_artifact_ids_for_basename,
    open_source_without_following_links, rotation_order, sha256_bytes,
};
...
pub enum SccmManifestCoverageScope {
    #[default]
    Source,
    /// Directory enumeration returned only a partial view of this configured
    /// root. This marker does not assert that any named rotation is missing.
    RootEnumeration,
}
...
pub fn write_sccm_manifest_v1(
    ...
    let mut output = options.open(&manifest_path)?;
...
fn read_legacy_manifest(bundle_root: &Path) -> Result<SccmBundleManifestV1, AppError> {
    ...
    let gaps = match legacy.pointer("/collection/results/gaps") {
        Some(Value::Array(gaps)) => gaps.as_slice(),
        Some(_) => {
            return Err(AppError::InvalidInput(
...
fn validate_native_manifest(
    ...
    if validate_files {
        for artifact in &manifest.artifacts {
            ...
            let mut input = open_source_without_following_links(&candidate).map_err(|error| {
                AppError::InvalidInput(format!(
                    "SCCM evidence {relative_path} cannot be opened safely: {error}"
                ))
            })?;
            ...
            {
                return Err(AppError::InvalidInput(format!(
                    "SCCM evidence {relative_path} violates opened-file coherence"
                )));
            }
            let content_sha256 = sha256_exact_file(&mut input, artifact.bytes_copied)?;
            if artifact.content_sha256.as_deref() != Some(content_sha256.as_str()) {
                return Err(AppError::InvalidInput(format!(
                    "SCCM evidence {relative_path} violates content digest coherence"
                )));
            }
        }
    }
...
fn validate_native_manifest_structure(manifest: &SccmBundleManifestV1) -> Result<(), AppError> {
    ...
    for gap in &manifest.capture_gaps {
        ...
        if !artifact_ids.insert(gap.artifact_id.clone()) {
            return Err(AppError::InvalidInput(
                "SCCM manifest contains duplicate artifact IDs".to_owned(),
            ));
        }
        let binding = (
            classify_artifact_name(&gap.basename, SccmRole::Client)
                .basename
                .to_ascii_lowercase(),
            gap.path_fingerprint.clone(),
        );
        if lineage_bindings
            .insert(gap.rotation_lineage.clone(), binding.clone())
            .is_some_and(|existing| existing != binding)
        {
            return Err(AppError::InvalidInput(
                "SCCM capture gap lineage crosses physical sources".to_owned(),
            ));
        }
    }
    if !manifest
...
fn validate_native_artifact(
    ...
    if physical {
        ...
        if artifact.coverage_scope == SccmManifestCoverageScope::RootEnumeration {
            ...
            validate_enumeration_provenance(artifact, &classified.basename)?;
        } else {
...
fn validate_capture_gap(gap: &SccmManifestCaptureGap) -> Result<(), AppError> {
    ...
    let classified = classify_artifact_name(&gap.basename, SccmRole::Client);
    if !classified.supported_for_diagnosis
        || classified.rotation != gap.rotation
        || gap.catalog_entry_id != catalog_entry_id(&classified.basename)
        || gap.logical_artifact_ids != logical_artifact_ids_for_basename(&classified.basename)
    {
        return Err(AppError::InvalidInput(
            "SCCM capture gap is not bound to the authoritative catalog".to_owned(),
        ));
    }
    let expected_source_digest = source_identity_digest(&gap.root_handle, &classified.basename)
...
fn validate_enumeration_provenance(
    ...
    let source_handle = artifact.source_handle.as_deref().ok_or_else(|| {
        AppError::InvalidInput("SCCM enumeration provenance is incomplete".to_owned())
    })?;
    let root_handle = artifact.root_handle.as_deref().ok_or_else(|| {
        AppError::InvalidInput("SCCM enumeration provenance is incomplete".to_owned())
    })?;
    let fingerprint = artifact.path_fingerprint.as_deref().ok_or_else(|| {
        AppError::InvalidInput("SCCM enumeration provenance is incomplete".to_owned())
    })?;
    let lineage = artifact.rotation_lineage.as_deref().ok_or_else(|| {
        AppError::InvalidInput("SCCM enumeration provenance is incomplete".to_owned())
    })?;
...
pub(super) fn compare_manifest_capture_gaps(
...
fn create_real_bundle_root(bundle_root: &Path) -> Result<std::path::PathBuf, AppError> {
    if !bundle_root.exists() {
        ...
        {
            return Err(AppError::InvalidInput(
                "SCCM bundle root must be pre-created with a restricted inherited ACL".to_owned(),
            ));
        }
        #[cfg(unix)]
        {
            ...
            builder.recursive(true).mode(0o700).create(bundle_root)?;
        }
...
fn verify_real_bundle_root(bundle_root: &Path) -> Result<std::path::PathBuf, AppError> {
    ...
    #[cfg(unix)]
    {
        ...
        if metadata.permissions().mode() & 0o077 != 0 {
            return Err(AppError::InvalidInput(
                "SCCM bundle root is not private".to_owned(),
            ));
        }
    }
...
fn root_handle_digest(root_handle: &str) -> Option<&str> {
