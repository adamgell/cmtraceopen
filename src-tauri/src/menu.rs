...
/// 8 lowercase hex chars derived from the entry's dedup key (normalized path +
/// workspace). `DefaultHasher` is not stable across Rust releases, which is
/// fine here: the hash is generated by `recent_menu_id` and verified by
/// `enrich_recent_payload` within a single process run, and is never
/// persisted.
