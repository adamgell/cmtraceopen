...
const STOPWORDS: &[&str] = &[
    ...
    "further", "then", "once", "and", "but", "or", "nor", "not", "no", "so", "if", "than",
    "too", "very", "just", "about", "up", "it", "its", "this", "that", "these", "those",
    "i", "me", "my", "we", "our", "you", "your", "he", "him", "his", "she", "her", "they",
    "them", "their", "what", "which", "who", "whom", "all", "each", "every", "both",
...
pub fn dbscan_cluster(
    ...
    for i in 0..n {
        ...
        let neighbors = region_query(embeddings, i, distance_threshold);
        ...
        while idx < seed_set.len() {
            ...
            let q_neighbors = region_query(embeddings, q, distance_threshold);
    ...
    for (&cid, chunk_indices) in &cluster_chunks {
        // Collect all entry IDs from chunks in this cluster
        ...
        // Find representative entry (closest to centroid)
        ...
        // Generate label from frequent tokens
    ...
    // Anomaly entry IDs = entries from noise chunks
    let mut anomaly_ids: Vec<u64> = noise_indices
        ...
        .into_iter()
        .collect();
    // Remove any entry IDs that also appear in a cluster
    let clustered_ids: HashSet<u64> = clusters
        .iter()
        .flat_map(|c| c.entry_ids.iter().copied())
        .collect();
    anomaly_ids.retain(|id| !clustered_ids.contains(id));
    anomaly_ids.sort();

    (clusters, anomaly_ids)
}

/// Returns indices of all points within `distance_threshold` cosine distance of point `idx`.
fn region_query(embeddings: &[Vec<f32>], idx: usize, distance_threshold: f32) -> Vec<usize> {
    let target = &embeddings[idx];
    embeddings
        .iter()
        .enumerate()
        .filter(|(_, emb)| {
            let sim = cosine_similarity(target, emb);
            (1.0 - sim) <= distance_threshold
        })
        .map(|(i, _)| i)
        .collect()
}
