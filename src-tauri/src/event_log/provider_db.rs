...
impl ProviderDb {
    ...
    fn rows_for(&self, provider_name: Option<&str>) -> Result<Vec<CapturedProviderMetadata>, String> {
        ...
        rows.map(|row| {
            let (provider_name, version_key, events, messages, tasks, keywords, opcodes, maps,
                parameters, source_os_build) =
                row.map_err(|error| format!("cannot read provider row: {error}"))?;
    ...
    for definition in definitions {
        if definition.get("Name").and_then(serde_json::Value::as_str) == Some("levels") {
            if let Some(values) = definition.get("Values") {
                return serde_json::from_value(values.clone()).map_err(|error| {
                    format!("provider ValueMapDefinition levels are not a map: {error}")
                });
            }
        }
    }
