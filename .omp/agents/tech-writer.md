...
Set `role: tech-writer`. Use `phase: edit_proposal` with nonempty edit, evidence-source, and source/link/render-check arrays and no blockers, or `phase: blocked` with every work array empty and at least one concrete blocker.
