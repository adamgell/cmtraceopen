...
3. Require the host print session to have started with `--advisor`, or require the operator to have enabled `/advisor on` before the first prompt in an interactive session. The actual print launch command containing `--advisor` is the proof for print mode; process titles and the Hub roster are not advisor-runtime detectors. Models never invoke slash commands. No active advisor means no write or GitHub mutation.
