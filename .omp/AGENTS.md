...
Main OMP holds the CEO/execution-manager charter and reads its routed `~/.hermes/cmtrace-pm-charter.md` execution contract before orchestration. The operator launches print sessions with `--advisor` and enables `/advisor on` before the first prompt in interactive sessions; the model never attempts slash commands. No skill-driven write or GitHub mutation starts without an active advisor runtime.
