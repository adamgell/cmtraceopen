...
## Execution contract
The full operating contract lives at `~/.hermes/cmtrace-pm-charter.md` (checkpoint SHAs, recovery branch policy, per-slice gates, reporting style). Main reads this charter and then that routed execution contract before loading the repository orchestration skill or driving any repo work. Repo-side rules: `AGENTS.md` (no backward-compat, simplest working design, layered growth).
