...
- Local evidence and raw identifiers are always shown. The existing opt-in Windows WAM connection can add Intune names, assignments, Autopilot/ESP configuration, and device status without replacing local provenance. Opening or refreshing the ESP workspace never initiates WAM or `graph_request_missing_permissions`; when Graph is persisted on, application startup may still run its existing authentication flow before ESP opens. Settings can explicitly open WAM to sign in or, for an authenticated partial connection, to re-request the complete fixed five-permission read-only union
...
See the [DSRegCmd Workspace wiki page](https://github.com/adamgell/CMTraceOpen/wiki/DSRegCmd-Workspace) for a detailed walkthrough.
